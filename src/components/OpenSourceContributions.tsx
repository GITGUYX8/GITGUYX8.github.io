...
const STATIC_PRS: Record<FilterType, PR[]> = {
  merged: [
    ...
    },
    {
      ...
      repository: { nameWithOwner: "GITGUYX8/aws-deploy" },
      state: "MERGED",
    ...
    },
    {
      ...
      repository: { nameWithOwner: "GITGUYX8/rviz2-nvnc-pipeline" },
      state: "MERGED",
    ...
    },
    {
      ...
      repository: { nameWithOwner: "GITGUYX8" },
      state: "MERGED",
...
export function OpenSourceContributions({ isFullPage = false }: { isFullPage?: boolean }) {
  ...
  const fetchPRsForType = useCallback(async (type: FilterType) => {
    // Static export: serve curated data, no GitHub token needed.
    setPrsByType(prev => ({ ...prev, [type]: STATIC_PRS[type] }));
        ...
        >
          {!isLoaded && currentPrs.length === 0 ? (
            ...
            <div className="flex justify-center py-4 text-center text-[13px] text-zinc-500">
              No contributions in this state yet.
            </div>
          )}
        </div>
      </div>
