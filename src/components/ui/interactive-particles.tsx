...
void main() {
  ...
  // Keep darker particles nearly full-size so the whole portrait renders even
  // at small display sizes (bright areas still read a touch larger).
  psize *= mix(0.62, 1.0, grey);
  psize *= uSize;
...
void main() {
  ...
  // Particles carry the tint colour directly (so dark areas of the source still
  // read as visible light specks on a dark backdrop); brightness drives opacity
  // instead, so the image reproduces as a tonal field rather than vanishing.
  vec3 rgb = uColor;
  float alpha = t * (0.2 + 0.8 * grey);
